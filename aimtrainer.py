...
def draw_top_bar(win, elapsed_time, targets_pressed, misses):
    ...
    name_label = LABEL_FONT.render("By: Arun", 1, "black") 
...
def end_screen(win, elapsed_time, targets_pressed, clicks):
    ...
    name_label = LABEL_FONT.render("Game by: Arun", 1, "white")

    win.blit(time_label, (get_middle(time_label), 100))
    win.blit(speed_label, (get_middle(speed_label), 200))
    win.blit(hits_label, (get_middle(hits_label), 300))
    win.blit(accuracy_label, (get_middle(accuracy_label), 400))
    win.blit(name_label, (get_middle(name_label), 500))
    ...
    while run:
        ...
        for target in targets:
            ...
            if target.size <= 0:
                ...
                misses += 1
                miss_sound.play()  # Play miss sound when a target disappears

            if click and target.collide(*mouse_pos):
                targets.remove(target)
                targets_pressed += 1
                hit_sound.play()  # Play hit sound when the target is clicked
